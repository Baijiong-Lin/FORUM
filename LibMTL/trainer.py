...
class Trainer(nn.Module):
    ...
    def _prepare_optimizer(self, optim_param, scheduler_param):
        ...
        self.ul_alpha_optimizer = torch.optim.Adam(self.ul_alpha.parameters(), lr=1e-3)
    ...
    def train_MOML(self, train_dataloaders, test_dataloaders, epochs, eta, val_dataloaders=None, return_weight=False, org_train_dataloaders=None):
        ...
        for epoch in range(epochs):
            ...
            for batch_index in range(train_batch):
                ...
                for tn in range(self.task_num):
                    ...
                    alpha_grad[tn] = - eta * g_F_tn @ g_f.t() @ g_s
                loss_data = torch.tensor([loss.item() for loss in losses]).to(self.device)
                alpha_grad = self.model.aggregated_grad(alpha_grad, loss_data, **self.kwargs['weight_args']) # --weighting EW/MGDA/MoCo
                del meta_model
    ...
    def train_FORUM(self, train_dataloaders, test_dataloaders, 
                epochs, rho, eta, inner_step, val_dataloaders=None, return_weight=False):
        r'''The training process of multi-task learning.
        ...
        for epoch in range(epochs):
            ...
            for batch_index in range(train_batch):
                ...
                inner_optim = torch.optim.SGD(inner_model.parameters(), lr=eta, weight_decay=0)
