...
def main(params):
    ...
    train_loader = DataLoader(train_dataset, batch_size=int(params.bs/2), shuffle=True, num_workers=2, pin_memory=True)

    if params.method in ['MOML']:
    ...
    QM9model = QM9trainer(task_dict=task_dict, 
                          ...
                          **kwargs)
    if params.method == 'FORUM':
        QM9model.train_FORUM(train_loader, test_loader, params.epochs, val_dataloaders=val_loader,
                            rho=params.rho,
                            eta=params.eta,
                            inner_step=params.inner_step)
    elif params.method == 'MOML':
        QM9model.train_MOML(train_loader, test_loader, params.epochs, val_dataloaders=val_loader,
                            eta=params.eta, org_train_dataloaders=org_train_loader)
    else:
        raise ValueError
